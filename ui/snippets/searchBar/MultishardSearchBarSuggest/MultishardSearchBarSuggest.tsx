...
import type { MiltishardSearchResults, SearchResultItem } from 'types/api/search';
...
interface Props {
  query: UseQueryResult<MiltishardSearchResults>;
  searchTerm: string;
  onItemClick: (event: React.MouseEvent<HTMLAnchorElement>) => void;
  containerId: string;
}

const MultishardSearchBarSuggest = ({ query, searchTerm, onItemClick, containerId }: Props) => {
  ...
  const data = React.useMemo(() => {
    const shards = getFeaturePayload(configs.features.search)?.shards;

    return shards?.reduce((acc: Array<SearchResultItem> | undefined, shard: Shard) => {
      // Return the shard data if it exists
      const result = query.data?.[shard.id].data;
      if (result?.length) {
        // each item should has a shard property
        const items = result.map((item) => ({ ...item, shard: shard }));
  ...
  const itemsGroups = React.useMemo(() => {
    if (!data?.length && !marketplaceApps.displayedApps) {
      return {};
    }
    ...
    return map;
  }, [ data, marketplaceApps.displayedApps ]);
...
export default MultishardSearchBarSuggest;
